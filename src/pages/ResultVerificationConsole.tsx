...
const ResultVerificationConsole: React.FC = () => {
  ...
  const [selectedPanelForDetail, setSelectedPanelForDetail] = useState<PanelRow | null>(null);
  ...
  const handleCardClick = (row: PanelRow, event: React.MouseEvent) => {
    ...
    setSelectedPanelForDetail(row);
    // Ensure analytes are loaded for the detail view
    ensureAnalytesLoaded(row.result_id);
  };
  ...
  const PanelCard: React.FC<{ row: PanelRow }> = ({ row }) => {
    ...
    return (
      ...
      onClick={(e) => handleCardClick(row, e)}>
        {/* Header */}
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              <input
                ...
                onChange={(e) => {
                  e.stopPropagation();
                  togglePanelSelection(row.result_id);
                }}
        ...
        <div className="px-6 py-2 bg-gray-50 border-t border-gray-100">
          <div className="text-xs text-gray-500 text-center">
            Click card to view detailed information • Use checkboxes for bulk operations
          </div>
        </div>
      </div>
    );
        ...
        <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-hidden">
            ...
            <div className="bg-gradient-to-r from-blue-600 to-indigo-600 px-8 py-6 text-white">
              <div className="flex items-center justify-between">
                <div>
                  <h2 className="text-2xl font-bold mb-2">Panel Details</h2>
                  <div className="flex items-center space-x-4 text-blue-100">
                    <span className="flex items-center">
                      <User className="h-4 w-4 mr-2" />
                      {selectedPanelForDetail.patient_name}
                    </span>
                    <span className="flex items-center">
                      <TestTube className="h-4 w-4 mr-2" />
                      {selectedPanelForDetail.test_group_name}
                    </span>
                    <span className="flex items-center">
                      <Calendar className="h-4 w-4 mr-2" />
                      {fmtDate(selectedPanelForDetail.order_date)}
                    </span>
                  </div>
                </div>
                <button
                  onClick={() => setSelectedPanelForDetail(null)}
            ...
            <div className="p-8 overflow-y-auto max-h-[70vh]">
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      <div className="text-3xl font-bold text-blue-900">
                        {selectedPanelForDetail.expected_analytes}
                      </div>
                      <div className="text-sm font-medium text-blue-700">Total Analytes</div>
                    </div>
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      <div className="text-3xl font-bold text-green-900">
                        {selectedPanelForDetail.approved_analytes}
                      </div>
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      <div className="text-3xl font-bold text-amber-900">
                        {selectedPanelForDetail.entered_analytes - selectedPanelForDetail.approved_analytes}
                      </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    ...
                    <div className="font-mono text-lg font-bold text-gray-900">
                      #{selectedPanelForDetail.order_id.slice(-8)}
                    </div>
                  </div>
                  <div>
                    <div className="text-sm text-gray-600 mb-1">Patient ID</div>
                    <div className="font-mono text-lg font-bold text-gray-900">
                      {selectedPanelForDetail.patient_id.slice(-8)}
                    </div>
                  </div>
                  <div>
                    <div className="text-sm text-gray-600 mb-1">Test Group</div>
              ...
              <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
                ...
                {rowsByResult[selectedPanelForDetail.result_id] ? (
                  <div className="overflow-x-auto">
                    <table className="min-w-full">
                      ...
                      <tbody className="divide-y divide-gray-100">
                        {(rowsByResult[selectedPanelForDetail.result_id] || []).map((a) => (
                          <tr key={a.id} className="hover:bg-gray-50 transition-colors">
                            ...
                            <td className="px-6 py-4">
                              <div className="font-bold text-xl text-gray-900">{a.value ?? "—"}</div>
                            </td>
                            <td className="px-6 py-4">
                              <span className="font-medium text-gray-700">{a.unit}</span>
                            </td>
                            ...
                            <td className="px-6 py-4">
                              {a.flag && (
                                <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-bold border ${
                                  a.flag === 'H' ? 'bg-red-100 text-red-800 border-red-200' :
                                  a.flag === 'L' ? 'bg-blue-100 text-blue-800 border-blue-200' :
                                  a.flag === 'C' || a.flag === 'Critical' ? 'bg-orange-100 text-orange-800 border-orange-200' :
                                  'bg-gray-100 text-gray-800 border-gray-200'
                                }`}>
                                  {a.flag === 'H' ? 'High' : a.flag === 'L' ? 'Low' : a.flag === 'C' ? 'Critical' : a.flag}
                                </span>
                              )}
                            </td>
                            <td className="px-6 py-4">
                              {a.verify_status === "approved" ? (
                                ...
                                <span className="inline-flex items-center px-4 py-2 rounded-lg text-sm font-semibold bg-red-100 text-red-800 border border-red-200">
                                  <XCircle className="h-4 w-4 mr-2" />
                                  Rejected
                                </span>
                              ) : (
                                <span className="inline-flex items-center px-4 py-2 rounded-lg text-sm font-semibold bg-amber-100 text-amber-800 border border-amber-200">
                            ...
                            <td className="px-6 py-4">
                              <div className="flex items-center space-x-2">
                                ...
                                {a.verify_status !== "rejected" && (
                                  <button
                                    ...
                                    className="inline-flex items-center px-3 py-2 rounded-lg text-sm font-semibold bg-gradient-to-r from-red-100 to-rose-100 text-red-700 hover:from-red-200 hover:to-rose-200 transition-all duration-200 shadow-sm border border-red-200 disabled:opacity-50"
                                  >
                              ...
                              {a.verify_note && (
                                <div className="text-xs text-gray-500 mt-2 italic bg-gray-50 p-2 rounded border max-w-xs">
                                  Note: {a.verify_note}
                                </div>
                              )}
                            </td>
                  ...
                  <div className="p-8 text-center">
                    ...
                    <div className="text-gray-600">Loading analyte details...</div>
                  </div>
                )}
              </div>
              ...
              <div className="flex items-center justify-between pt-6 border-t border-gray-200">
                <div className="text-sm text-gray-600">
                  {selectedPanelForDetail.approved_analytes}/{selectedPanelForDetail.expected_analytes} analytes approved
                </div>
                <div className="flex items-center space-x-4">
                  <button
                    onClick={() => setSelectedPanelForDetail(null)}
                  ...
                  {selectedPanelForDetail.approved_analytes < selectedPanelForDetail.expected_analytes && (
                    <button
                      disabled={busy[selectedPanelForDetail.result_id]}
                      onClick={() => {
                        approveAllInPanel(selectedPanelForDetail);
                        setSelectedPanelForDetail(null);
                      }}
                      className="inline-flex items-center px-6 py-3 bg-gradient-to-r from-green-600 to-emerald-600 text-white rounded-xl hover:from-green-700 hover:to-emerald-700 transition-all duration-200 shadow-lg font-semibold disabled:opacity-50"
                    >
                      {busy[selectedPanelForDetail.result_id] ? (
                        <Loader2 className="h-5 w-5 mr-2 animate-spin" />
                      ) : (
                        <CheckCircle2 className="h-5 w-5 mr-2" />
                      )}

-- SAFER VERSION: Fix for Order Status and Sample Collection Status Synchronization
-- This migration ensures order status stays in sync with sample collection status
-- Fixed to avoid foreign key constraint issues
...
BEGIN
  ...
  IF TG_OP = 'UPDATE' THEN
    ...
    IF OLD.sample_collected_at IS NULL AND NEW.sample_collected_at IS NOT NULL THEN
      ...
      -- Log the status change (only if we have a valid user)
      IF current_user_id IS NOT NULL THEN
...
BEGIN
  ...
  RAISE NOTICE '✅ Added safe audit logging for status changes';
  RAISE NOTICE '';
  ...
  RAISE NOTICE '- Log status synchronization actions when user context is available';
END $$;